...
сканирование → предпросмотр → назначение 7 ролей → сохранение mapping,
...
class CameraCalibrationApiTest(unittest.TestCase):
    ...
    def test_scan_not_enough_cameras(self):
        ...
        self.assertIn("Открылось камер: 2/7", state["error"])
    ...
    def test_scan_keeps_available_after_error(self):
        api = self.make_api(set(range(4)))
        api.scan()
        self.assertEqual(api.status, "ERROR")
        self.assertEqual(api.available_cameras, [0, 1, 2, 3])
    ...
    def test_full_assignment_flow(self):
        ...
        self.assertEqual(len(api.assignments), 7)
        self.assertTrue(all(
    ...
    def test_get_frame_camera_failure(self):
        ...
        api = calib.CameraCalibrationApi(
            self.config, scan_limit=7, capture_factory=factory,
        )
        api.scan()
        self.assertEqual(api.status, "READY")
        result = api.get_frame()
    ...
    def test_state_shape(self):
        ...
        self.assertEqual(len(state["roles"]), 7)
