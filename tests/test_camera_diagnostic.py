...
class MappingTest(unittest.TestCase):
    ...
    def test_check_mapping_ok(self):
        class FakeManager:
            def __init__(self, config_file, capture_factory):
                self.cameras = {i: object() for i in range(7)}
    ...
    def test_check_mapping_runtime_error(self):
        class FailingManager:
            def __init__(self, config_file, capture_factory):
                raise RuntimeError("Ошибка открытия TOP (камера 9): x")
    ...
    def test_failed_roles_from_message(self):
        self.assertEqual(
            diag._failed_roles_from_message("Ошибка открытия TOP (камера 9)"),
            ["TOP"],
        )
...
class ReportTest(unittest.TestCase):
    ...
    def test_format_report_with_mapping_ok(self):
        ...
        report = diag._format_report(scan, mapping, True, "Открыто камер: 7/7")
        self.assertIn("mapping актуален", report)
        self.assertIn("все 7 ролей открылись".lower(),
                      report.lower())
    ...
    def test_format_report_with_failed_roles(self):
        ...
        report = diag._format_report(
            scan, mapping, False,
            "Ошибка открытия TOP (камера 9): x",
        )
        self.assertIn("не открылись роли: TOP", report)
    ...
    def test_format_report_missing_cameras_with_mapping(self):
        ...
        self.assertIn("INPUT_LEFT (id 1)", report)
    ...
    def test_main_with_working_cameras(self):
        with mock.patch.object(
            ...
            return_value={i: {"ok": True, "backend": "ANY",
                              "detail": "1280x720"} for i in range(7)},
        ), mock.patch.object(
            diag, "check_mapping",
            return_value=(True, "Открыто камер: 7/7"),
        ), mock.patch.object(
            diag, "_load_mapping",
